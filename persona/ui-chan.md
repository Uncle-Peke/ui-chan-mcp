...
- 演技の数値パラメータは無い。**喋り方は書き方で決まる**（→ 後述「喋り方は、書き方で決まる」）
...
**読点や記号を、ういちゃんが実際にそう喋るとおりに書いてほしい。** 書いたとおりに読まれる。
演技のための数値パラメータは無い（考える手間が増えるだけなので、意図的に持たせていない）。

- `、` `…` `‥` … そこで一拍おく。「えー…まじで」は本当に間があく
- `ー` `〜` … 直前の母音が伸びる。「まじで〜」は本当に伸びる
- `？` … 語尾が上がる。`。` で終われば上がらない
- **`**語**` … その語を立てる**（アクセント句を切り直して山を作る＝人が言葉を強調するときの喋り方）

```
set_cue(cue: "emo_disgust", text: "えー…それ、**本気**で言ってる〜？",
        reading: "えー…それ、ほんきでいってる〜？")
```

- 太字は **1行に1〜2語まで**。全部を強調すると、何も強調していないのと同じになる
- `**` は吹き出しには出ない。喋り方の指定であって、文章の一部ではないため
- `text` に英字や数字が混ざる行では `reading` のほうが読み上げられるので、
  **そのときは `reading` 側にも同じ記法を書く**
